...
import cn.tgc.demo.service.ISiteService;
...
public class AdminController {
    ...
    public String index(HttpServletRequest request) {
        ...
        StatisticsBo staticticsBo = siteServiceImpl.getStatistics();
...
//向凭论列表页面跳转
        @Autowired
        private ICommentService commentServiceimp;
        @GetMapping(value = "/comment")
        public String index1(@RequestParam(value = "page", defaultValue = "1") int page,
                            @RequestParam(value = "count", defaultValue = "10") int count,
                            HttpServletRequest request) {
            PageInfo<Comment> commentPageInfo = commentServiceimp.selectArticleWithPage(page, count);
            request.setAttribute("comment", commentPageInfo);
       ...
       /* @GetMapping(value = "/comment/{id}")
        public String editComment(@PathVariable("id") String id, HttpServletRequest request) {
            Comment comment = commentServiceimp.selectCommentWithId(Integer.parseInt(id));
            request.setAttribute("COMMENT", comment);
           *//* request.setAttribute("categories", article.getCategories());*//*
            return "back/updateComment";
        }*/

    //文章删除
    @PostMapping(value = "/comment/delete")
    @ResponseBody
    public ArticleResponseData comdelete(@RequestParam int id) {
        try {
            commentServiceimp.deleteArticleWithIda(id);
            logger.info("文章删除成功");
            return ArticleResponseData.ok();
        } catch (Exception e) {
            logger.error("文章删除失败，错误信息: "+e.getMessage());
